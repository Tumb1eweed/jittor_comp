#!/usr/bin/env bash
# Train only the zero-initialized normal/tangent stage-2 gates.
set -euo pipefail

ROOT=/home/PGD
PYTHON_BIN=/root/miniconda3/envs/jittor/bin/python3.7
OUT_ROOT=${OUT_ROOT:-"$ROOT/experiments/score82_p2_dual_gate/round300"}
STEPS=${STEPS:-300}
...
mkdir -p "$OUT_ROOT/dual_gate_score"
CUDA_VISIBLE_DEVICES=${CUDA_DEVICE:-2} "$PYTHON_BIN" "$ROOT/tools/train_shapenet_one_epoch.py" \
  --use_cuda \
  ...
  --pgd_second_stage_surface_k 16 --pgd_train_stage2_dual_gate_only \
  --pgd_composite_loss \
  ...
  --seed 8300 --step_metrics_every 10 \
  --log_dir "$OUT_ROOT/dual_gate_score" >"$OUT_ROOT/dual_gate_score/train.log" 2>&1
