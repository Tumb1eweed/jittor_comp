#!/usr/bin/env bash
# Surface-aware local-uniformity continuation.  This reads only the frozen
# train_90 split; the fixed train holdout is reserved for later screening.
set -euo pipefail

ROOT=/home/PGD
PYTHON_BIN=/root/miniconda3/envs/jittor/bin/python3.7
ROUND_ROOT=${ROUND_ROOT:-"$ROOT/experiments/score82_tangent_spacing_round"}
mkdir -p "$ROUND_ROOT"
...
common=(
  ...
  --tangent_spacing_points 192 --tangent_spacing_k 8 --tangent_spacing_collapse_margin 0.90
  --corr_huber_delta 0.01 --step_metrics_every 10
)

# Three branches deliberately occupy currently free GPU pairs while the
# separate OT holdout screen runs on GPUs 4,5.  The zero-weight branch makes
# the stronger continuation schedule distinguishable from the new loss.
names=(control tangent005 tangent010)
devices=(0,1 2,3 6,7)
weights=(0.00 0.05 0.10)
pids=()
for i in "${!names[@]}"; do
  out="$ROUND_ROOT/${names[$i]}"
  mkdir -p "$out"
  "$PYTHON_BIN" "$ROOT/tools/launch_manual_multigpu.py" --devices "${devices[$i]}" --log_dir "$out" -- \
    "${common[@]}" --loss_tangent_spacing_weight "${weights[$i]}" \
    --seed "$((9501 + i))" --log_dir "$out" >"$out/launcher.log" 2>&1 &
  pids+=("$!")
done
status=0
