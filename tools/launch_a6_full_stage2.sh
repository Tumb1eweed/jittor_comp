#!/usr/bin/env bash
# A6: specialize the complete non-shared stage-2 encoder/decoder/backbone.
# It is copied from the submitted shared backbone at load time, so step zero
# exactly reproduces the current best two-stage prediction.
set -euo pipefail
...
OUT=${OUT:-"$ROOT/experiments/score82_a6_full_stage2/round100"}
...
CUDA_VISIBLE_DEVICES="$DEVICE" "$PYTHON_BIN" "$ROOT/tools/train_shapenet_one_epoch.py" \
  ...
  --lr 0.000005 --lr_schedule warmup_cosine \
  --lr_warmup_steps 20 --lr_warmup_start 0.0000005 --lr_min 0.0000005 \
  ...
  --pgd_use_separate_stage2 --pgd_train_separate_stage2_only \
  ...
  --corr_huber_delta 0.01 --seed 8305 --step_metrics_every 10 \
  --log_dir "$OUT"
