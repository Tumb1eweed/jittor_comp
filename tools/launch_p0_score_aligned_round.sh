#!/usr/bin/env bash
# Train-only P0 sweep: scorer-aligned CD plus noisy-normalized paired
# normal/tangent correspondence. Validation is deliberately not opened here.
set -euo pipefail
...
common=(
  ...
  --patch_size 1500 --patches_per_shape 4 --batch_size "$BATCH_SIZE"
  --epochs 1 --train_steps_per_epoch "$TRAIN_STEPS" --val_steps_per_epoch 0
  --lr 0.00001 --lr_schedule warmup_cosine
  --lr_warmup_steps 30 --lr_warmup_start 0.000002 --lr_min 0.000002
  ...
  --corr_huber_delta 0.01 --seed 8280 --step_metrics_every 10
)

p0_common=(
  --pgd_composite_loss
  --loss_relative_weight 0.5
  --loss_pred_cd_weight 0.25 --loss_clean_cd_weight 0.5
  --loss_score_relative_weight 0.5
  --loss_infocd_weight 0.05 --loss_uniform_weight 0.02
  --loss_stage_weight 0.20
)

names=(control_plain score_only corr025 corr_tangent2)
devices=(0 2 4 6)
pids=()

for i in "${!names[@]}"; do
  if [[ -n "${BRANCH_FILTER:-}" && ",${BRANCH_FILTER}," != *",${names[$i]},"* ]]; then
    continue
  fi
  out="$ROUND_ROOT/${names[$i]}"
  mkdir -p "$out"
  if [[ "${names[$i]}" == "control_plain" ]]; then
    CUDA_VISIBLE_DEVICES=${devices[$i]} "${common[@]}" --log_dir "$out" \
      >"$out/train.log" 2>&1 &
  elif [[ "${names[$i]}" == "score_only" ]]; then
    CUDA_VISIBLE_DEVICES=${devices[$i]} "${common[@]}" "${p0_common[@]}" \
      --loss_corr_weight 0.0 --log_dir "$out" >"$out/train.log" 2>&1 &
  elif [[ "${names[$i]}" == "corr025" ]]; then
    CUDA_VISIBLE_DEVICES=${devices[$i]} "${common[@]}" "${p0_common[@]}" \
      --pgd_use_normal_corr_loss --pgd_normal_corr_relative \
      --normal_corr_normal_weight 2.0 --normal_corr_tangent_weight 1.0 \
      --loss_corr_weight 0.25 --log_dir "$out" >"$out/train.log" 2>&1 &
  else
    CUDA_VISIBLE_DEVICES=${devices[$i]} "${common[@]}" "${p0_common[@]}" \
      --pgd_use_normal_corr_loss --pgd_normal_corr_relative \
      --normal_corr_normal_weight 1.0 --normal_corr_tangent_weight 2.0 \
      --loss_corr_weight 0.25 --log_dir "$out" >"$out/train.log" 2>&1 &
  fi
  pids+=("$!")
done
