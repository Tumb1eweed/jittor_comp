#!/usr/bin/env bash
# A1 phase 1: learn the StraightPCF-style remaining-distance module while
# preserving the current best PGD velocity field and backbone.
set -euo pipefail
...
OUT=${OUT:-"$ROOT/experiments/score82_a1_surface_flow/distance300"}
...
CUDA_VISIBLE_DEVICES="$DEVICE" "$PYTHON_BIN" "$ROOT/tools/train_shapenet_one_epoch.py" \
  ...
  --lr 0.0002 --lr_schedule warmup_cosine \
  --lr_warmup_steps 20 --lr_warmup_start 0.00002 --lr_min 0.00002 \
  ...
  --pgd_use_surface_flow --pgd_train_surface_flow_distance_only \
  --pgd_surface_flow_train_intermediate \
  --pgd_surface_flow_max_time 0.8 --pgd_surface_flow_start_prob 0.5 \
  --pgd_composite_loss \
  ...
  --pgd_surface_flow_velocity_weight 0.0 \
  --pgd_surface_flow_distance_weight 0.50 \
  --corr_huber_delta 0.01 --seed 8301 --step_metrics_every 10 \
  --log_dir "$OUT"
