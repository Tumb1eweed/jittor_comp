#!/usr/bin/env bash
# Strong, train-only CD-loss continuation sweep.  The fixed 26-shape split is
# used only after training for model selection; validation data is never read.
set -euo pipefail
...
extras=(
  "--loss_anti_cluster_weight 1.0 --anti_cluster_k 8 --anti_cluster_margin 0.90"
  "--loss_density_weight 1.0"
  "--loss_local_surface_weight 0.5"
  "--loss_anti_cluster_weight 0.5 --anti_cluster_k 8 --anti_cluster_margin 0.90 --loss_density_weight 0.5 --loss_local_surface_weight 0.25"
)
pids=()
for i in "${!names[@]}"; do
  name=${names[$i]}; out="$ROUND_ROOT/$name"; mkdir -p "$out"
  read -r -a extra <<< "${extras[$i]}"
  "$PYTHON_BIN" "$ROOT/tools/launch_manual_multigpu.py" --devices "${devices[$i]}" --log_dir "$out" -- \
    "${common[@]}" "${extra[@]}" --seed "$((9100+i))" --log_dir "$out" >"$out/launcher.log" 2>&1 &
  pids+=("$!")
done
status=0
