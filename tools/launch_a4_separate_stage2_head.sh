#!/usr/bin/env bash
# A4: specialize a non-shared stage-2 displacement head.  The complete stage-2
# network is initialized from the old shared backbone, so step zero reproduces
# the current checkpoint exactly; only its final displacement MLP is trained.
set -euo pipefail
...
OUT=${OUT:-"$ROOT/experiments/score82_a4_separate_stage2/head100"}
...
CUDA_VISIBLE_DEVICES="$DEVICE" "$PYTHON_BIN" "$ROOT/tools/train_shapenet_one_epoch.py" \
  ...
  --lr 0.00005 --lr_schedule warmup_cosine \
  --lr_warmup_steps 20 --lr_warmup_start 0.000005 --lr_min 0.000005 \
  ...
  --pgd_use_separate_stage2 --pgd_train_separate_stage2_head_only \
  --pgd_train_detach_second_stage_features \
  ...
  --corr_huber_delta 0.01 --seed 8303 --step_metrics_every 10 \
  --log_dir "$OUT"
