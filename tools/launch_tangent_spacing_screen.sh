#!/usr/bin/env bash
# Fixed-training-holdout selector for tangent-plane spacing candidates.
set -euo pipefail
...
common=(
  ...
  --workers 8 --python "$PYTHON_BIN" --model pgd --datalist_dir "$ROOT/datalist"
  --val_list "$HOLDOUT_LIST" --sample_points 50000 --noise_std_min 0.005 --noise_std_max 0.020
  --seed 8200 --patch_size 1500 --seed_k 7 --seed_k_alpha 10 --patch_batch_size 8 --patch_fusion select
