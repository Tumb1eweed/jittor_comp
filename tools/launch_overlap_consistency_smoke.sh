#!/usr/bin/env bash
# Train-only smoke test for overlap displacement consistency.  Evaluation uses
# the separate train holdout created by make_stratified_train_holdout.py.
set -euo pipefail

ROOT=/home/PGD
PYTHON_BIN=/root/miniconda3/envs/jittor/bin/python3.7
OUT=${OUT:-"$ROOT/experiments/score82_overlap_smoke"}
mkdir -p "$OUT"
...
exec "$PYTHON_BIN" "$ROOT/tools/launch_manual_multigpu.py" --devices 6,7 --log_dir "$OUT" -- \
  ...
  --epochs 1 --train_steps_per_epoch 200 --val_steps_per_epoch 0 \
  ...
  --corr_huber_delta 0.01 --seed 8300 --step_metrics_every 10 --log_dir "$OUT"
