...
def local_normals(points, indices):
    ...
    normals = vectors[:, :, 0]
    return (normals / np.maximum(np.linalg.norm(normals, axis=1, keepdims=True), 1e-8)).astype(np.float32)


def repel(pred, noisy, k, margin, step, iterations, normal_source):
    tree = cKDTree(noisy)
    ...
    normals = local_normals(normal_points, indices)
    ...
    for _ in range(int(iterations)):
        ...
        out = out + float(step) * force
    return out.numpy().astype(np.float32), scale
...
def main():
    ...
    for pred_path in pred_paths[args.shard_index::args.num_shards]:
        ...
        corrected, scale = repel(
            pred, noisy, args.k, args.margin, args.step, args.iterations, args.normal_source
        )
    ...
    print("processed={}, shard={}/{}, median_scale={:.6f}".format(
        len(scales), args.shard_index, args.num_shards, median
    ))
