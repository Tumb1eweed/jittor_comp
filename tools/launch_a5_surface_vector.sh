#!/usr/bin/env bash
# A5: learn a directly supervised residual surface-correction vector on top of
# the current two-stage PGD.  The final layer is zero-initialised, so step zero
# exactly reproduces the current best checkpoint.
set -euo pipefail
...
OUT=${OUT:-"$ROOT/experiments/score82_a5_surface_vector/round100"}
...
CUDA_VISIBLE_DEVICES="$DEVICE" "$PYTHON_BIN" "$ROOT/tools/train_shapenet_one_epoch.py" \
  ...
  --pgd_use_surface_vector_head --pgd_train_surface_vector_only \
  --pgd_surface_vector_hidden_dim 64 --pgd_surface_vector_max_distance 0.02 \
  --pgd_surface_vector_unit_slope \
  --pgd_surface_vector_nearest_target \
  --pgd_composite_loss \
  --loss_corr_weight 0.0 --loss_relative_weight 0.10 \
  --loss_pred_cd_weight 0.10 --loss_clean_cd_weight 0.10 \
  --loss_score_relative_weight 0.20 \
  --loss_infocd_weight 0.05 --loss_uniform_weight 0.01 \
  --loss_stage_weight 0.0 \
  --pgd_surface_vector_residual_weight 1.0 \
  --pgd_surface_vector_tangent_weight 0.10 \
  --corr_huber_delta 0.01 --seed 8304 --step_metrics_every 10 \
  --log_dir "$OUT"
